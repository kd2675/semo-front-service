...
export function ScheduleClient({ clubId, clubName, months, isAdmin = false }: ScheduleClientProps) {
  ...
  const [selectedDay, setSelectedDay] = useState(months[0]?.defaultSelectedDay ?? 1);
  ...
  const selectMonth = (direction: "prev" | "next") => {
    startTransition(() => {
      ...
      setSelectedDay(months[nextIndex]?.defaultSelectedDay ?? 1);
    });
  };
            ...
            <div className="mb-2 grid grid-cols-7 text-center">
              ...
              {Array.from({ length: month.daysInMonth }, (_, index) => {
                ...
                return (
                  ...
                  >
                    {isActive ? (
                      <div className="flex size-8 items-center justify-center rounded-full bg-[var(--primary)] font-bold text-white shadow-lg shadow-[var(--primary)]/30">
                        {day}
                      </div>
                    ) : (
                      <span>{day}</span>
