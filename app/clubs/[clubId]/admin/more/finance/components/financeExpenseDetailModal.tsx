          ...
          {editing && posted ? (
            <section className="space-y-4 rounded-[var(--radius-modal)] border border-orange-200 bg-orange-50/40 p-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="금액"><input aria-label="정정할 지출 금액" value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" className={FIELD_CLASS} /></Field>
                <Field label="카테고리"><select aria-label="정정할 지출 카테고리" value={categoryCode} onChange={(event) => setCategoryCode(event.target.value)} className={FIELD_CLASS}>{CATEGORY_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field>
                <Field label="재정 기간"><select aria-label="정정할 지출 재정 기간" value={financePeriodId} onChange={(event) => setFinancePeriodId(event.target.value)} className={FIELD_CLASS}><option value="">날짜 기준 자동 선택</option>{operations.periods.filter((period) => period.statusCode === "OPEN").map((period) => <option key={period.financePeriodId} value={period.financePeriodId}>{period.title}</option>)}</select></Field>
                <Field label="지출 계좌"><select aria-label="정정할 지출 계좌" value={financeAccountId} onChange={(event) => setFinanceAccountId(event.target.value)} className={FIELD_CLASS}><option value="">기본 지출 계좌</option>{operations.accounts.filter((account) => account.active && account.usageScopeCode !== "COLLECTION").map((account) => <option key={account.financeAccountId} value={account.financeAccountId}>{account.displayName}</option>)}</select></Field>
                <Field label="연결 일정"><select aria-label="정정할 지출 연결 일정" value={scheduleEventId} onChange={(event) => setScheduleEventId(event.target.value)} className={FIELD_CLASS}><option value="">일정 연결 안 함</option>{operations.scheduleOptions.map((schedule) => <option key={schedule.eventId} value={schedule.eventId}>{schedule.startAtLabel} · {schedule.title}</option>)}</select></Field>
