...
import {
  updateClubAdminScheduleSettings,
...
type ClubAdminScheduleSettingsClientProps = {
  clubId: string;
  initialData: ClubAdminScheduleSettingsResponse;
};

type SchedulePermissionKey = "allowMemberCreate" | "allowMemberUpdate" | "allowMemberDelete";

type SchedulePermissionCardProps = {
  icon: string;
  title: string;
  description: string;
  enabled: boolean;
...
function SchedulePermissionCard({
  icon,
  title,
  description,
  enabled,
  onChange,
}: SchedulePermissionCardProps) {
  return (
...
export function ClubAdminScheduleSettingsClient({
  clubId,
  initialData,
}: ClubAdminScheduleSettingsClientProps) {
  ...
  const handleToggle = (key: SchedulePermissionKey, next: boolean) => {
    setDraft((current) => ({
      ...current,
      [key]: next,
    }));
  ...
  const handleSave = async () => {
    setSaving(true);
    clearToast();
    const result = await updateClubAdminScheduleSettings(clubId, {
    ...
    if (!result.ok || !result.data) {
      showToast(result.message ?? "일정 권한 설정 저장에 실패했습니다.", "error");
      return;
    }

    setDraft(result.data);
    setSaved(result.data);
    showToast("일정 권한 설정이 저장되었습니다.", "success");
  };
        ...
        <main className="semo-nav-bottom-space space-y-4 px-4 pt-4">
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">
              Schedule Permission Policy
            </p>
            <h1 className="mt-3 text-2xl font-bold tracking-tight">멤버 일정 작업 범위를 조정합니다.</h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">
              아래 설정은 일반 멤버에게만 적용됩니다. 관리자와 오너는 이 설정과 무관하게 일정을
              계속 생성, 수정, 삭제할 수 있습니다.
            </p>
          </motion.section>

          <motion.section {...staggeredFadeUpMotion(1, reduceMotion)}>
            <SchedulePermissionCard
              icon="event_available"
              title="멤버 일정 생성 허용"
              description="사용자가 자기 일정을 새로 작성할 수 있게 합니다."
          ...
          <motion.section {...staggeredFadeUpMotion(2, reduceMotion)}>
            <SchedulePermissionCard
              icon="edit"
              title="멤버 본인 일정 수정 허용"
              description="사용자가 본인이 작성한 일정만 수정할 수 있게 합니다."
          ...
          <motion.section {...staggeredFadeUpMotion(3, reduceMotion)}>
            <SchedulePermissionCard
              icon="delete"
              title="멤버 본인 일정 삭제 허용"
              description="사용자가 본인이 작성한 일정을 직접 삭제할 수 있게 합니다."
