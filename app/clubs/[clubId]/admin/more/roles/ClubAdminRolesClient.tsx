...
import { adminRoleHistoryQueryOptions, adminRoleManagementQueryOptions, roleQueryKeys } from "@/app/lib/react-query/roles/queries";
...
function RoleHistoryCard({
  ...
  onDelete: (positionHistoryId: number) => void;
  deleting: boolean;
}) {
  return (
    <article className="rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-slate-900">{history.memberDisplayName}</p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-[#fff1e4] px-2.5 py-1 text-[11px] font-bold text-[var(--secondary)]">
        ...
        <button
          type="button"
          disabled={deleting}
          onClick={() => onDelete(history.positionHistoryId)}
          className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-600 transition hover:bg-red-100 disabled:opacity-50"
        >
      ...
      <div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2">
        <span>시작: {history.startedAtLabel ?? "기록 없음"}</span>
        <span>종료: {history.endedAtLabel ?? "현재"}</span>
      </div>
...
export function ClubAdminRolesClient({ clubId, initialData }: ClubAdminRolesClientProps) {
  ...
  const handleDeleteHistory = async (positionHistoryId: number) => {
    if (!window.confirm("잘못 등록된 직책 보유 이력을 삭제할까요? 현재 직책 배정은 멤버 관리에서 별도로 조정해야 합니다.")) {
      return;
    }
    await deleteHistoryMutation.mutateAsync(positionHistoryId);
    await queryClient.invalidateQueries({ queryKey: roleQueryKeys.adminRoleHistory(clubId) });
  };
    ...
    >
      <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(255,220,194,0.7),_transparent_28%),linear-gradient(180deg,_#f7fafc_0%,_#f2f5f7_100%)] pb-28">
        ...
        <main className="mx-auto max-w-7xl px-6 py-8">
          ...
          <motion.section className="mt-10" {...staggeredFadeUpMotion(7, reduceMotion)}>
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-slate-400">Tenure History</p>
                <h3 className={`${manrope.className} mt-2 text-2xl font-black tracking-tight text-slate-900`}>
                  직책 보유 기간
                </h3>
              </div>
            </div>
            {roleHistoryQuery.isPending ? (
              <div className="rounded-[24px] bg-white p-5 text-sm text-slate-500 shadow-sm">직책 이력을 불러오는 중입니다.</div>
            ) : roleHistoryQuery.isError ? (
              <div className="rounded-[24px] bg-red-50 p-5 text-sm text-red-600">직책 이력을 불러오지 못했습니다.</div>
            ) : roleHistoryQuery.data.histories.length === 0 ? (
              <div className="rounded-[24px] bg-white p-5 text-sm text-slate-500 shadow-sm">아직 기록된 직책 보유 이력이 없습니다.</div>
            ) : (
              <div className="grid gap-3 lg:grid-cols-2">
                {roleHistoryQuery.data.histories.slice(0, 8).map((history) => (
                  <RoleHistoryCard
                    key={history.positionHistoryId}
                    history={history}
                    deleting={deleteHistoryMutation.isPending}
                    onDelete={(positionHistoryId) => void handleDeleteHistory(positionHistoryId)}
                  />
                ))}
              </div>
            )}
