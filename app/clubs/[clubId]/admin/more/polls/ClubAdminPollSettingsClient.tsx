...
type ClubAdminPollSettingsClientProps = {
  clubId: string;
  initialData: ClubAdminPollSettingsResponse;
};

type PollPermissionKey = "allowMemberCreate" | "allowMemberUpdate" | "allowMemberDelete";

type PollPermissionCardProps = {
  icon: string;
  title: string;
  description: string;
  enabled: boolean;
...
function PollPermissionCard({
  icon,
  title,
  description,
  enabled,
  onChange,
}: PollPermissionCardProps) {
  return (
...
export function ClubAdminPollSettingsClient({
  clubId,
  initialData,
}: ClubAdminPollSettingsClientProps) {
  ...
  const handleToggle = (key: PollPermissionKey, next: boolean) => {
    setDraft((current) => ({
      ...current,
      [key]: next,
    }));
  ...
  const handleSave = async () => {
    setSaving(true);
    clearToast();
    const result = await updateClubAdminPollSettings(clubId, {
    ...
    if (!result.ok || !result.data) {
      showToast(result.message ?? "투표 권한 설정 저장에 실패했습니다.", "error");
      return;
    }

    setDraft(result.data);
    setSaved(result.data);
    showToast("투표 권한 설정이 저장되었습니다.", "success");
  };
        ...
        <main className="semo-nav-bottom-space space-y-4 px-4 pt-4">
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">
              Poll Permission Policy
            </p>
            <h1 className="mt-3 text-2xl font-bold tracking-tight">멤버 투표 작업 범위를 조정합니다.</h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">
              아래 설정은 일반 멤버에게만 적용됩니다. 관리자와 오너는 이 설정과 무관하게 투표를
              계속 생성, 수정, 삭제할 수 있습니다.
            </p>
          </motion.section>
          ...
          <motion.section {...staggeredFadeUpMotion(1, reduceMotion)}>
            <PollPermissionCard
              icon="ballot"
              title="멤버 투표 생성 허용"
              description="사용자가 자기 투표를 새로 작성할 수 있게 합니다."
          ...
          <motion.section {...staggeredFadeUpMotion(2, reduceMotion)}>
            <PollPermissionCard
              icon="edit"
              title="멤버 본인 투표 수정 허용"
              description="사용자가 본인이 작성한 투표만 수정할 수 있게 합니다."
          ...
          <motion.section {...staggeredFadeUpMotion(3, reduceMotion)}>
            <PollPermissionCard
              icon="delete"
              title="멤버 본인 투표 삭제 허용"
              description="사용자가 본인이 작성한 투표를 직접 삭제할 수 있게 합니다."
