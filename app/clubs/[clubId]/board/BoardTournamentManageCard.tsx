...
function getTournamentSecondaryText(tournament: TournamentSummary) {
  const parts = [
    ...
    <div className="relative overflow-visible rounded-[8px] border border-slate-100 bg-white shadow-sm">
      ...
      >
        <div className="mb-2 flex items-center justify-between gap-3">
          ...
          <span className="shrink-0 text-xs text-slate-400">{tournament.startDate}</span>
        </div>
        <h2 className="mb-2 line-clamp-1 text-base font-bold text-slate-900">{tournament.title}</h2>
        <p className="line-clamp-2 text-sm leading-6 text-slate-500">
          {tournament.summaryText ?? getTournamentSecondaryText(tournament)}
        </p>
