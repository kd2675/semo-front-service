...
- 고정 크기 중앙 코어 하단의 가로형 규모 세모 0~5개: 현재 활성 멤버 수
- 중앙 코어 주변 반짝임 1~5개: 최근 14일간 기록된 성공 동작의 절대 활동 수준
- 바깥 삼각의 세 꼭짓점: `함께`, `운영`, `이어짐`의 다음 티어 진행
- 세 축이 모두 임계치를 넘으면 소재가 한 단계 올라가고, 초과분만 남긴 작은 삼각으로 다음 단계를 시작

소재와 삼각 진행은 클럽의 품질, 인기도, 회원 수, 경쟁 순위를 뜻하지 않습니다. 코어 보석은 멤버 수와 무관하게 고정 크기이며, 활성 멤버 규모는 보석 하단의 작고 채워진 세모를 가로로 나열해 표시합니다. `0명=0개`, `1~4명=1개`, `5~19명=2개`, `20~49명=3개`, `50~99명=4개`, `100명 이상=5개`를 사용합니다. 이는 기존 7단계에서 인접 구간을 합치되 100명 이상 최종 구간을 보존한 기준입니다. 정확한 회원 수는 기존 숫자 정보로도 제공합니다. 회원 수는 소재, 세 축 점수, 검색 정렬, 가입 추천 가중치, 기능 잠금 해제, 금전적 보상에 영향을 주지 않습니다.
...
점수는 활동 로그의 `subject`나 사람이 읽는 `detail_text`를 파싱하지 않습니다. 각 업무 원장의 현재 상태를 집계합니다. 활동 로그는 최근 활동 표시에만 사용하며 `0`, `1~3`, `4~9`, `10~24`, `25 이상`을 API의 `activityLevel` 0~4단계로 바꿉니다. 화면은 이 값을 반짝임 1~5개로 일대일 변환하고 보석의 광도는 바꾸지 않습니다. 이는 절대 활동량이라 회원이 많은 클럽에 반짝임이 더 많을 수 있으며, 그 차이는 소재 티어나 추천 순위로 전이되지 않습니다.

갱신 계약:

- 모임 생성 트랜잭션 안에서 첫 성장 보석 상태를 함께 저장합니다.
...
- 루트 홈의 내 클럽 카드: 대표 이미지 위에 고정 크기 보석·규모 세모 수·반짝임 수·소재색 표시
- 루트 홈의 공개 클럽 탐색: 고정된 80px 무대에 보석·규모 세모 수·반짝임 수·소재색을 주 시각 정보로 표시
- 공개 클럽 소개 모달: 바깥 성장 삼각과 세 축 진행을 노출하지 않고 보석·규모 세모·반짝임만 크게 표시
- `/clubs/{clubId}` 홈: 상단 클럽 커버 오른쪽에 보석·규모 세모·반짝임을 요약한 성장 표식을 표시하고, 선택하면 `/clubs/{clubId}/growth` 상세 화면으로 이동
- `/clubs/{clubId}/growth`: 코어와 바깥 성장 삼각을 포함한 필수 성장 상세 패널 표시
- 바깥 삼각의 현재 단계와 세 축 진행은 클럽 내부 성장 상세 화면에서만 표시합니다.
- 다음 소재까지의 진행은 `0/3`의 실선 시작 삼각과 33%·66%·100%에 놓인 세 개의 점선 목표 삼각으로 표시합니다. 화면 상태는 총 4단계이며, 각 축의 현재 꼭짓점은 `0~32`에서 시작 삼각 꼭짓점을 유지하고, `33~65`면 1단계 점선, `66~99`면 2단계 점선, `100`이면 3단계 점선 꼭짓점에 위치합니다. 점선 사이의 중간 위치는 사용하지 않습니다. 세 축이 모두 100%가 되면 다음 소재로 올라가고 새 소재의 `0/3` 시작 삼각으로 돌아갑니다.
- 목록의 여러 코어에는 반복 애니메이션을 사용하지 않습니다.
- 상세 코어의 별형 반짝임만 저속 opacity·scale 모션을 사용할 수 있고 `prefers-reduced-motion`에서는 정지합니다. 목록의 반짝임은 개수만 보이는 정적 표시입니다.
- 목록·공개 소개 SVG에는 소재·멤버 규모 세모 수·활동 반짝임 수를, 내부 성장 SVG에는 소재·멤버 규모 세모 수·세 축 진행·활동 반짝임 수를 설명하는 접근성 이름을 제공합니다.
- 티어 표식이 있는 모든 화면은 표식 전체를 44px 이상의 네이티브 버튼 또는 링크로 제공합니다. 목록·공개 소개에서는 클릭·탭·키보드 선택 시 동일한 설명 UI를 열고, 클럽 커버 표식은 `/clubs/{clubId}/growth` 상세 화면으로 이동합니다. 설명 UI와 상세 화면에는 현재 티어, 활성 멤버 수와 규모 세모 구간, 최근 14일 성공 기록과 반짝임 구간, 티어 순서, 순위·인기도가 아니라는 안내를 포함합니다.
- 목록의 각 티어마다 `표시 설명` 배지를 반복하지 않습니다. 티어 표면 전체를 설명 트리거로 유지하고 평상시에는 보석을 우선하며, hover·focus-visible에서만 테두리와 포커스 링을 보여주고 데스크톱 도움말 커서를 사용합니다.
- 이미 `RouteModal` 안에 있는 공개 클럽 소개에서는 새 모달을 중첩하지 않고 동일한 모달의 본문을 설명 화면으로 전환하며, 뒤로 가기와 전체 닫기를 모두 제공합니다.
...
- `useHydrationSafeReducedMotion` 또는 공용 motion 정책
