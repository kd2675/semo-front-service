...
- 루트 홈의 내 클럽 카드: 대표 이미지 위에 크기·빛·소재색만 가진 작은 코어 보석 표시
- 루트 홈의 공개 클럽 탐색: 고정된 80px 무대에 크기·빛·소재색만 가진 코어 보석을 주 시각 정보로 표시
- 공개 클럽 소개 모달: 바깥 삼각과 세 축 진행을 노출하지 않고 코어 보석만 크게 표시
- `/clubs/{clubId}` 홈: 설정 가능한 위젯보다 앞에 코어와 바깥 성장 삼각을 포함한 필수 성장 패널 표시
- 바깥 삼각의 현재 단계와 세 축 진행은 클럽 내부 성장 패널에서만 표시합니다.
- 다음 소재까지의 진행은 33%·66%·100%에 놓인 세 개의 점선 삼각으로 표시합니다. 세 축의 최솟값이 `0~32`면 1단계, `33~65`면 2단계, `66~99`면 3단계이며 세 축이 모두 100%가 되면 다음 소재로 올라가고 1단계부터 다시 시작합니다.
- 목록의 여러 코어에는 반복 애니메이션을 사용하지 않습니다.
- 상세 코어만 내부 맥박을 사용할 수 있고 `prefers-reduced-motion`에서는 정지합니다.
- 목록·공개 소개 SVG에는 소재·멤버 크기·활동 밝기를, 내부 성장 SVG에는 소재·세 축 진행·활동 밝기를 설명하는 접근성 이름을 제공합니다.
...
- `ClubGrowthCoreMark`는 특정 모임의 멤버 수, 활동, 성장 기록을 표현하는 데이터 표식입니다. 외부 목록과 공개 소개에서는 `core-only`, 클럽 내부 성장 패널에서는 전체 성장 프레임을 사용하며 앱 로고나 일반 페이지 아이콘으로 대체하지 않습니다.
