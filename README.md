...
- 사용자 화면에서 운영자 모드로 들어가는 동선은 각 페이지의 플로팅 버튼을 반복하지 않고 하단/데스크톱 More 메뉴의 `운영자 모드로 전환` 항목으로 제공합니다.
...
- 모바일은 하단 내비게이션을 유지하고 `1280px` 이상에서는 같은 정보 구조를 왼쪽 세로 레일로 전환해 넓은 화면의 하단 공백과 포인터 이동 거리를 줄입니다.
...
- 독립 콘텐츠 객체와 편집 단위만 카드로 구분합니다. 내비게이션·설정·운영 작업은 `semo-list`/`semo-list-row`, 묶음 지표는 `semo-metric-strip`, 페이지 대표 정보는 화면당 하나의 `semo-feature-surface`를 우선해 카드 중첩을 피합니다.
- 공용 반경·그림자·간격·동작 시간 token을 우선하며, 본문·보조 정보는 최소 `text-xs`를 기준으로 유지합니다.
- 공통 진입 모션은 정보를 숨기지 않는 짧은 이동과 낮은 지연만 사용하고, 반복·강한 모션은 성장 코어와 실제 상태 변화처럼 의미가 있는 요소에 제한합니다.
...
- 2026-09-02 확인
  ...
  - `npm run verify:ui` 성공 (66개 라우트, 213개 TSX UI 계약 검사, 11px 텍스트 0건)
  - `npm run build` 성공
